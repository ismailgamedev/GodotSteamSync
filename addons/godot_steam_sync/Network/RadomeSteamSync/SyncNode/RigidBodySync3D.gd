...
func _on_timer_timeout():
	if get_parent().position != last_pos and NetworkManager.GAME_STARTED:
		var DATA : Dictionary = {"Idx":packet_index,"player_id":NetworkManager.STEAM_ID,"TYPE":NetworkManager.TYPES.RIGIDBODY_SYNC,"value":[get_parent().linear_velocity,get_parent().angular_velocity,get_parent().position,deg_to_rad(get_parent().rotation_degrees)],"node_path":get_path()}
...
func update_physics_values():
	...
	get_parent().rotation_degrees = lerp_angle(deg_to_rad(get_parent().rotation_degrees) , target_rotation, lerp_speed)
